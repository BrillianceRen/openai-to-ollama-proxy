...
基于 Google 官方 google-genai SDK 把 Google Cloud Vertex AI
转换为 Ollama API, 同时提供 OpenAI 兼容接口 /v1/chat/completions 与 /v1/models。
支持 API Key、Application Default Credentials (ADC adc.json) 与 OAuth2 Bearer Token。
...
----
    ...
    python vertex-ollama-proxy.py --vertex-project <PROJECT_ID>
...
class VertexClient:
    """基于 google-genai SDK 的 Google Cloud Vertex AI 客户端。"""
    ...
    def __init__(self, config):
        ...
        self._sig_cache_file = os.path.join(SCRIPT_DIR, "tmp", "thought_signatures_vertex.json")
    ...
    def get_sdk_client(self):
        if self._sdk_client is None:
            if self.config.credentials_file and os.path.exists(self.config.credentials_file):
                os.environ["GOOGLE_APPLICATION_CREDENTIALS"] = self.config.credentials_file
            elif not os.environ.get("GOOGLE_APPLICATION_CREDENTIALS"):
                appdata = os.environ.get("APPDATA", "")
                home = os.path.expanduser("~")
                candidate_adcs = [
                    os.path.join(SCRIPT_DIR, "adc.json"),
            ...
            client_kwargs = {
                "vertexai": True,
                "project": self.config.vertex_project or None,
                "location": self.config.vertex_location or DEFAULT_VERTEX_LOCATION,
                "http_options": {"headers": headers} if headers else None,
            }
            if self.config.api_key:
                client_kwargs["api_key"] = self.config.api_key

            self._sdk_client = genai.Client(**client_kwargs)
    ...
    def fetch_models(self):
        ...
        try:
            ...
            for m in client.models.list():
                ...
                clean = raw_name.split("/")[-1] if raw_name.startswith("publishers/google/models/") or raw_name.startswith("models/") else raw_name
                # 过滤掉 TTS、纯音频、纯图像检测、转写等非对话推理模型
                if any(x in clean.lower() for x in ("-tts", "native-audio", "-image", "embedding", "transcribe", "efficientnet", "yolo", "deeplab", "sentiment", "syntax", "resnet", "vit-", "pt-test")):
                    continue
                if clean and clean not in models:
                    models.append(clean)
            if models:
                self.log("通过 google-genai 成功拉取 %d 个 Vertex 模型: %s" % (
            ...
            self.log("google-genai 拉取 Vertex 模型列表跳过: %s" % exc, level="debug")
    ...
    def warm_models(self):
        ...
        def _probe():
            try:
                ...
                self.log("Vertex 初始化探测成功, 可用模型数: %d" % len(models))
                        ...
                        if resp.candidates and resp.candidates[0].content:
                            for p in resp.candidates[0].content.parts:
                                ...
                                if sig:
                                    ...
                                    self.log("Vertex 思考签名预热成功", level="debug")
                                    break
                    except Exception as sig_err:
                        self.log("Vertex 思考签名预热跳过: %s" % sig_err, level="debug")
    ...
    def normalize_model_name(self, name):
        ...
        if base.startswith("publishers/google/models/"):
            base = base[len("publishers/google/models/"):]
    ...
    def sanitize_schema(raw):
        ...
        for k, v in raw.items():
            ...
            if k == "properties" and isinstance(v, dict):
                cleaned["properties"] = {pk: VertexClient.sanitize_schema(pv) for pk, pv in v.items()}
            elif k == "items":
                if isinstance(v, dict):
                    cleaned["items"] = VertexClient.sanitize_schema(v)
                elif isinstance(v, list):
                    cleaned["items"] = [VertexClient.sanitize_schema(x) for x in v]
                else:
                    cleaned["items"] = v
            elif k in ("anyOf", "any_of") and isinstance(v, list):
                cleaned["any_of"] = [VertexClient.sanitize_schema(x) for x in v]
                ...
                cleaned[k] = VertexClient.sanitize_schema(v)
            elif isinstance(v, list):
                cleaned[k] = [VertexClient.sanitize_schema(x) for x in v]
            else:
                cleaned[k] = v
        if not cleaned:
            return {"type": "object"}
    ...
    def chat(self, body):
        ...
        try:
            ...
            self.log("Vertex SDK 错误: %s" % exc, level="error")
    ...
    def chat_stream(self, body, write):
        ...
        try:
            ...
            self.log("Vertex 流式错误: %s" % exc, level="error")
            raise UpstreamError(500, str(exc)) from exc
...
def main():
    parser = argparse.ArgumentParser(description="Google Cloud Vertex AI -> Ollama API 代理 (基于 google-genai SDK)")
    ...
    print("vertex-ollama-proxy 已启动: http://%s:%d (引擎: google-genai SDK)" % (config.host, config.port))
    proj_display = config.vertex_project or "(未设置，请在 config.json 中配置)"
    print("  - Vertex AI: Project=%s, Region=%s" % (proj_display, config.vertex_location))
    print("  - 默认模型: %s" % config.default_model)
    auth_info = ("ADC 文件: " + os.path.basename(config.credentials_file)) if config.credentials_file else (
        ("API Key: " + config.api_key[:6] + "..." + config.api_key[-4:]) if len(config.api_key) > 10 else "ADC 自动发现"
    )
